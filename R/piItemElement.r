piItemElement <- function(identifier = NULL, identifierType = NULL, object = NULL, exception = NULL) {
	...
	if (is.null(object) == FALSE) {
		...
		if ((is.null(className)) || (className != "piElement")) {
			return (print(paste0("Error: the class from the parameter object should be piElement.")))
		}
	}
	...
	attr(valueCleaned, "className") <- "piItemElement"
	return(valueCleaned)
}
