analysisCategoryApi <- R6Class("analysisCategoryApi",
	private = list(),
	public = list(
		serviceBase = NULL,
		authType = NULL,
		...
		getByPath = function(path, selectedFields) {
			...
			localVarPath <- paste(c(self$serviceBase, '/analysiscategories'), collapse = "")
			...
			if (res$status == 200) {
				attr(contentResponse, "className") <- "piAnalysisCategory"
			}
		...
		getSecurity = function(webId, userIdentity, forceRefresh, selectedFields) {
			...
			localVarPath <- paste(c(self$serviceBase, '/analysiscategories/', webId, '/security'), collapse = "")
		...
		getSecurityEntries = function(webId, nameFilter, selectedFields) {
			...
			localVarPath <- paste(c(self$serviceBase, '/analysiscategories/', webId, '/securityentries'), collapse = "")
		...
		getSecurityEntryByName = function(name, webId, selectedFields) {
			...
			localVarPath <- paste(c(self$serviceBase, '/analysiscategories/', webId, '/securityentries/', name), collapse = "")
			if (missing(selectedFields) == FALSE && is.null(selectedFields) == FALSE && selectedFields != "") {
				queryParameters$selectedFields <- selectedFields
				if (is.character(selectedFields) == FALSE) {
					return (print(paste0("Error: selectedFields must be a string.")))
