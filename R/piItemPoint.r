piItemPoint <- function(identifier = NULL, identifierType = NULL, object = NULL, exception = NULL) {
	...
	if (is.null(object) == FALSE) {
		...
		if ((is.null(className)) || (className != "piPoint")) {
			return (print(paste0("Error: the class from the parameter object should be piPoint.")))
		}
	}
	...
	attr(valueCleaned, "className") <- "piItemPoint"
	return(valueCleaned)
}
